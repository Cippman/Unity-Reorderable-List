...
namespace CippSharpEditor.Reorderable
{
    ...
    public class ReorderableUnityEventDrawer1 : PropertyDrawer
    {
        private static MethodInfo UnityEventBaseFindMethod = null;
        private static GUIContent GUIContentMixedValueContent = new GUIContent("—|Mixed Values");
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            ReorderableUnityEventAttribute reorderableAttribute = attribute as ReorderableUnityEventAttribute;
            
            if (reorderableAttribute == null)
            {
                GUI.Label(position, "ReorderableUnityEvent attribute not found.", EditorStyles.label);
                return;
            }
            ...
            List<UnityEventBaseReferences> referenceses = new List<UnityEventBaseReferences>();
            for (int i = 0; i < calls.arraySize; i++)
            {
                referenceses.Add(new UnityEventBaseReferences(calls.GetArrayElementAtIndex(i)));
            }

            reorderableAttribute.holder.references = referenceses;
            SerializedObject serializedListHolder = new SerializedObject(reorderableAttribute.holder);
            SerializedProperty references = serializedListHolder.FindProperty("references");
            if (references == null)
            {
                Debug.LogError("References not found!");
                return;
            }
            
            ReorderableList referencesReorderableList = new ReorderableList(references, true, true, true);
            serializedListHolder.Update();
            referencesReorderableList.DoList(EditorGUI.IndentedRect(position), label);
            height = referencesReorderableList.GetHeight();
            serializedListHolder.ApplyModifiedProperties();
//
//            if (UnityEventBaseFindMethod == null)
//            {
//                MethodInfo[] methods = typeof(UnityEventBase).GetMethods(BindingFlags.NonPublic | BindingFlags.Instance |
//                                                      BindingFlags.FlattenHierarchy);
//                foreach (MethodInfo method in methods)
//                {
//                    if (method.Name == "FindMethod" && method.GetParameters().Length == 4)
//                    {
//                        UnityEventBaseFindMethod = method;
//                        break;
//                    }
//                }
//            }
...
//            }
        ...
        private void DrawElementCallback(Rect rect, SerializedProperty element, GUIContent label, bool selected, bool focused)
        {
            Rect foldoutRect = rect;
            foldoutRect.height = EditorGUIUtility.singleLineHeight;
            element.isExpanded = EditorGUI.Foldout(foldoutRect, element.isExpanded, element.displayName);

            if (element.isExpanded)
            {
                //Rect[] rowRects = GetRowRects(rect);
                SerializedProperty callStateProperty = element.FindPropertyRelative("m_CallState");
                Rect callStateRect = foldoutRect;
                callStateRect.y += EditorGUIUtility.singleLineHeight+EditorGUIUtility.standardVerticalSpacing;
                EditorGUI.BeginChangeCheck();
                EditorGUI.PropertyField(callStateRect, callStateProperty, GUIContent.none);
...
//            }
//
//            return UnityEventBaseFindMethod.Invoke(dummyEvent, new object[] {methodName, (object) uObject, (object)modeEnum, argumentType}) != null;
